...
def switch_server_version(state: InstallerState) -> None:
    ...
    # 扫描 server 文件夹中的可用版本
    available_zips = list(config.SERVER_DIR.glob("*.zip"))
    if not available_zips:
        print("未找到任何服务端版本文件，请先下载。")
        return
    
    print("\n可用的本地服务端版本：")
    for idx, zip_file in enumerate(sorted(available_zips), start=1):
